...
void calcMotorSpeed(){
  ...
  //모터 속도 보정(호버링)//
  if(throttle > 225){
    throttle = throttle - 22;
  motorA_speed = (throttle == 0) ? 0 : throttle + yaw_output + roll_output + pitch_output - 19;
  } else{
    motorA_speed = (throttle == 0) ? 0 : throttle + yaw_output + roll_output + pitch_output - 19;
  }
  
  motorB_speed = (throttle == 0) ? 0 : throttle - yaw_output - roll_output + pitch_output + 20;
  motorC_speed = (throttle == 0) ? 0 : throttle + yaw_output - roll_output - pitch_output + 15;
  motorD_speed = (throttle == 0) ? 0 : throttle - yaw_output + roll_output - pitch_output - 19;
