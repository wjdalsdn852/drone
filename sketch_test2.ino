...
float roll_balancing_force;
float pitch_balancing_force;
float yaw_balancing_force;
...
void setup() {
  ...
  Serial.begin(115200); //HM-10(블루투스)로 부터 데이터를 받기 위해서 선언//
...
void mid_loop(int n) {
  ...
  calcMotorSpeed(n); //PID출력값을 구한것을 기준으로 모터의 속도를 계산한다.//  
  updateMotorSpeed();
}
...
void dualPID(float target_angle,
  ...
  balancing_force = (1*angle_error) + (1.5*(-rate_in)); 
  ...
  output = rate_pterm + rate_iterm + stabilize_iterm + balancing_force ; //최종 출력 : 각속도 비례항 + 각속도 적분항 + 안정화 적분항//
...
// 모터 회전 시키기
void servo_ratation() {
   myservo.write(180);    // 오른쪽으로 90의 속도로 회전
      delay(600);         // 회전 유지
   myservo.write(90);
   check_distance();      // 거리 계산 or 속도변경
   
   myservo.write(0);      // 왼쪽으로 90의 속도로 회전
        delay(600);       // 회전 유지
   myservo.write(90);
   check_distance();      // 거리 계산 or 속도변경
}
 ...
 // 거리 계산 or 속도변경
void check_distance() { 
  int limit_distance = 50;  // 제한 거리 
  int maintain_time = 3000; // 바꾼 후 지속 초/1000
  volt = map(analogRead(A0), 0, 1023, 0, 5000);     // 아날로그 0번 핀
  distance = (27.61 / (volt - 0.1696)) * 1000;
  //distance = 3.6473*(1/pow(volt,3)) - 9.6037*(1/pow(volt,2)) + 36.39*(1/volt) - 4.1143;

 if( 10 <= distance <= limit_distance ) {   // 측정 거리가 제한거리보다 적다면 실행
  mid_loop(1);
  // myservo.write(90);
  delay(maintain_time);   // 딜레이 주어서 변경한 속도로 잠시 유지
